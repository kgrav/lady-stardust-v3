...
public static class PhysicsExtensions
{
    ...
    public static void OverlapColliderNonAlloc(Collider collider, Collider[] results, int layerMask = Physics.DefaultRaycastLayers, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
	{
        if(collider.TryGetComponent(out CapsuleCollider capsuleCollider))
            OverlapCapsuleNonAlloc(capsuleCollider, results, layerMask, queryTriggerInteraction);
        else if (collider.TryGetComponent(out BoxCollider boxCollider))
            OverlapBoxNonAlloc(boxCollider, results, layerMask, queryTriggerInteraction);
        else if (collider.TryGetComponent(out SphereCollider sphereCollider))
            OverlapSphereNonAlloc(sphereCollider, results, layerMask, queryTriggerInteraction);
